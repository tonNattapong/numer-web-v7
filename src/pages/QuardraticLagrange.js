...
class QuardraticLagrange extends React.Component
{
    ...
    Lagrange(x,x0,x1,x2,y0,y1,y2){
            ...
            var y1
            var y2
            var result=(((x1-x)*(x2-x))/((x1-x0)*(x2-x0))*y0)+(((x0-x)*(x2-x))/((x0-x1)*(x2-x1))*y1)+(((x0-x)*(x1-x))/((x0-x2)*(x1-x2))*y2)
    ...
    handleSubmit(event){
        ...
        const result = this.Lagrange(x,x0,x1,x2,y0,y1,y2)
    ...
    render(){
        return(
          <Bo onSubmit={this.handleSubmit}>
            <Bg>
                <Title>&emsp;Quardratic Lagrange Interpolation&emsp;</Title>
